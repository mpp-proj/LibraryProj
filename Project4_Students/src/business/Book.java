...
final public class Book implements Serializable {
	...
	public boolean isAvailable() {
		return this.copies.length == 0 ? false : true;
	}
	...
	public BookCopy getNextAvailableCopy() {
		...
		if(getNumCopies() > 0)
		{
			tmp = copies[copies.length - 1];
			BookCopy[] bk = new BookCopy[copies.length - 1];
			System.arraycopy(copies, 0, bk, 0, copies.length - 1);
			copies = bk;
		}
		return tmp;
	}
